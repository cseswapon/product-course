...
const CourseInstructor: React.FC<CourseInstructorProps> = ({ section }) => {
  ...
  return (
    <Element name="instructors" className="my-5 md:mb-16">
      ...
      {values.map((item, idx) => (
        <div key={idx} className="flex items-center md:rounded-md md:border border-[#dbe1eb] md:p-5">
          <div className="rounded-[50%] overflow-hidden transition-opacity duration-300 ease-in-out">
            <Image
              src={item?.image || ""}
              alt={item?.slug || ""}
              width={100}
